import {
  ...
  import { contactCrudFactory } from './contact-crud';
  
  export const contactClient = {
    ...contactCrudFactory<Shop, QueryOptions, ShopInput>(API_ENDPOINTS.CONTACT),
  
    get({ slug }: { slug: String }) {
      return HttpClient.get<Shop>(`${API_ENDPOINTS.CONTACT}/${slug}`);
    },
    paginated: ({ name, ...params }: Partial<EmployeeQueryOptions>) => {
      return HttpClient.get<EmployeePaginator>(API_ENDPOINTS.CONTACT, {
        searchJoin: 'and',
        ...params,
    ...
    filter: (variables: ApproveShopInput) => {
      return HttpClient.post<any>(
        `${API_ENDPOINTS.CONTACT}/${API_ENDPOINTS.APPROVE_SHOP}`,
        variables);
    },
    ...
    disapprove: (variables: { id: string }) => {
      return HttpClient.post<{ id: string }>(
        `${API_ENDPOINTS.CONTACT}/${API_ENDPOINTS.DISAPPROVE_SHOP}`,
